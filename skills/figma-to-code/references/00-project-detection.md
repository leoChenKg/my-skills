...
| 审核方式 | —（不看项目约定） | **本 skill 写死**：步骤 2b 默认逐模块把 Figma 导出代码逐字存为 `preview/src/modules/<nodeIdSafe>.tsx`，登记模块 facts 并生成 `reference-preview.png`；交付代码保存 `implementation-preview.png`，由**人工对照预览截图 + 逐属性核对模块 `.tsx`/期望表**审核（无像素对比，见 05） |
...
node .agents/skills/figma-to-code/scripts/scan-components.mjs
