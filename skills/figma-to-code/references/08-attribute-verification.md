...
```text
整稿 source .tsx ──extract-spec.mjs --node-id(静态解析)──▶ 指定 node 子树期望属性表
                                              │
agent 生成的代码 ──────────────────────────┴──▶ agent 逐项比对 ──▶ 漏/错差异 ──▶ 修正 ──▶ 复跑
```

- **确定性在"支持模式的抽取"**：默认运行 `node scripts/extract-spec.mjs <source.tsx> --node-id <nodeId>`，从整稿 source `.tsx` 中只抽该 node 及其子树；fallback 时也可对稳定父级/模块 `.tsx` 运行。脚本对已支持的 Tailwind/Figma class 模式确定性抽出样式属性；未知、非标、未解析项必须进入清单由 agent 人工核对，不能假装已覆盖。agent 不再凭记忆列"该查哪些属性"，而是以脚本输出 + 未知项清单作为核对入口。
...
4. **layoutRisk(A7)**：`contents + left/top/right/bottom/inset/margin`、`relative contents`、或 metadata 显示 `contents` 节点有几何职责时输出风险。命中后，fallback 分模块导出只可作叶子属性参考，布局几何回到整稿/稳定父级导出与 `get_metadata` 坐标。

## 用法与闭环（3c-auto）

```text
1. node scripts/extract-spec.mjs .figma-to-code/preview/src/source/<targetNodeIdSafe>.tsx --node-id <nodeId>
...
4. 「非标/未知」逐个人判后落代码
5. 若输出 `layoutRisk`：记录到卡点；fallback 导出的风险几何不得落代码，改用整稿/父级导出与 `get_metadata` 坐标
6. 不可见层自检（F5）：装饰层 fill vs 父容器背景色色差，≈ 则回查真实色再填（见下「不可见层确定性自检」）
7. 全部对齐（或差异已在卡点说明）→ 进 3c 人工审核
```
...
- **不自动判定布局/定位/间距是否正确**（A4）：设计上排除，人工仍需做；脚本只对已知 `contents` 坏味道发出风险提示。
